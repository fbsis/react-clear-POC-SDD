...
- [X] T110 [US4] Add regression coverage and fix playback for a one-event battle so the first lethal attack transitions immediately to the completed result, clears transient attacking/impact states, never schedules a nonexistent next event and replaces the active Pause state with the correct completed controls in src/presentation/battle-playback/useBattlePlayback.ts, src/presentation/battle-playback/playbackReducer.ts, src/presentation/battle-playback/BattlePlaybackPage.tsx, src/presentation/battle-playback/useBattlePlayback.test.ts and tests/e2e/battle-playback.spec.ts
- [X] T111 [US1] Separate successful register/cleanup command outcomes from projection refresh failures, update the in-memory collection from command results and cover partial-failure behavior in src/app/providers/MonsterCollectionProvider.tsx and src/app/providers/MonsterCollectionProvider.test.tsx
- [X] T112 [US2] Prevent incompatible concurrent save, cleanup and battle-start operations and ignore stale battle completions after navigation or session reset using explicit session status and generation guards in src/app/contexts/GameSessionContextValue.ts, src/app/providers/GameSessionProvider.tsx, src/presentation/local-data/LocalDataManager.tsx, src/presentation/fighter-selection/FighterSelectionPage.tsx and their tests
- [X] T113 [US1] Validate catalog image existence inside RegisterMonster through the injected MonsterImageCatalog port before persistence and cover missing references in src/application/monster/RegisterMonster.ts, src/application/monster/RegisterMonster.test.ts and src/app/composition-root/createApplication.ts
- [X] T114 [US1] Validate the bundled catalog manifest, evict rejected fetch promises for retry and hydrate catalog and stored monsters independently in src/infrastructure/images/validateBundledCatalogRecords.ts, src/infrastructure/images/BundledMonsterImageCatalog.ts, src/infrastructure/images/BundledMonsterImageCatalog.test.ts and src/app/providers/MonsterCollectionProvider.tsx
- [X] T115 [US1] Replace swallowed upload, cleanup and monster-image failures with accessible inline errors and a finite missing-image state in src/presentation/monster-registration/MonsterRegistrationPage.tsx, src/presentation/local-data/LocalDataManager.tsx, src/presentation/shared/images/useMonsterImageUrl.ts, src/presentation/shared/images/MonsterImage.tsx and their tests
- [X] T116 [US2] Enforce runtime aggregate immutability and complete battle HP/order/damage invariants with dedicated validation and negative regression tests in src/domains/monster/Monster.ts, src/domains/monster/validations/validateMonsterCreatedAt.ts, src/domains/battle/validations/validateRound.ts, src/domains/battle/validations/validateBattle.ts and adjacent domain tests
